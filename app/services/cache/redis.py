...
class RedisCache(AbstractCache):
    """
    Кэш данных в Redis. Является singleton.

    Пример использования::

        from app.services.cache import RedisCache

        cache = RedisCache('localhost', 6379, 0, "password") # Создание объекта
        cache.set('key', 'value')
        cache.get('key') # Получение значения из кэша
        cache.delete('key') # Удаление значения из кэша
        cache.clear() # Очистка кэша.
        cache.delete_namespace("user") # Очистка кэша с указанным пространством имен. Если не указан, то очистка всех кэшей.
    """
