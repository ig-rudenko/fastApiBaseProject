...
from ..services.aaa import create_jwt_token_pair, get_current_user, refresh_access_token
from ..services.aaa.users import create_user, get_user_by_credentials, get_user_credentials
...
async def get_tokens(
    user_data: UserCredentialsSchema = Depends(get_user_credentials),
...
def verify_jwt(user: models.User = Depends(get_current_user)):
    """Получение данных пользователя (проверка валидности JWT)"""
    return user
